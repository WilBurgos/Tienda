...
<script type="text/javascript">
    $(document).ready(function () {
        var table               = $('#tableUser');
        var routeIndexAdmin     = "{!! route('clientes.index') !!}";
        var routeUsuarios       = "{!! route('clientes.get_clientes') !!}";
        var routeStoreUser      = "{!! route('clientes.store') !!}";
        var modal               = $('#modalRegister');
        var divNuevo            = '<div style="position:relative; margin-top:10px; margin-bottom:10px; float:left!important;"><button class="btn btn-secondary" type="button" id="nuevoUsuario">Nuevo Cliente</button></div>';
            ...
            table.bootstrapTable({
                ...
                columns: [{
                    ...
                    formatter: (value, row, index, field) => {
                        if(row.nombre){
                            return row.nombre+' '+row.primerAp+' '+row.segundoAp;
                        } else {
                            return 'Pendiente...';
                        }			
                    },
        ...
        window.operateEvents = {
            'click #edit': function(e, value, row, index){
                e.preventDefault();
                limpiarModal();
                tituloModal.append('Actualizar Cliente');
                $('#formUpdateUser').show();
                $('#formNewUser').hide();
                ...
                // --------------------------------------------- //
                $('#upd-id').val(row.id);
                $('#upd-name').val(row.nombre);
                $('#upd-primerAp').val(row.primerAp);
                $('#upd-segundoAp').val(row.segundoAp);
                modal.modal('show');
            }
        }
